...
function renderStaffGroups(staffRows, branches) {
  ...
  const branchNames = getOrderedBranchNames(branches, activeStaff);

  therapistGrid.innerHTML = branchNames.map((branchName) => {
    const branchStaff = activeStaff.filter((staff) => isSameBranch(staff.branch, branchName));
    const femaleStaff = branchStaff.filter((staff) => matchesGender(staff.gender, "female"));
    const maleStaff = branchStaff.filter((staff) => matchesGender(staff.gender, "male"));
    const sections = [
...
function renderTherapistProfile(staff) {
  ...
  const avatar = imageUrl
    ...
    : `<div class="therapist-avatar therapist-avatar-placeholder">${escapeHtml(getInitials(name))}</div>`;
  const profileUrl = `${getBranchPagePath("therapist")}?name=${encodeURIComponent(name)}`;
